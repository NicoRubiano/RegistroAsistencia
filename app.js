(() => {
  const KEY = "fet_asistencia_v6";
  const SESSION_KEY = "fet_asistencia_session";
  const channel = "BroadcastChannel" in window ? new BroadcastChannel("fet_asistencia_sync") : null;

  const seed = {
    university: { name:"Universidad Demo", graceMinutes:10, sessionMinutes:20 },
    users:[
      {id:"u_admin", role:"admin", name:"Administrador General", email:"admin@universidad.edu", password:"1234"},
      {id:"u_p1", role:"professor", name:"Laura Gómez", email:"laura.gomez@universidad.edu", password:"1234"},
      {id:"u_p2", role:"professor", name:"Carlos Ruiz", email:"carlos.ruiz@universidad.edu", password:"1234"},
      {id:"u_s1", role:"student", name:"Ana Torres", email:"ana.torres@universidad.edu", password:"1234", studentCode:"20261001"},
      {id:"u_s2", role:"student", name:"Mateo Díaz", email:"mateo.diaz@universidad.edu", password:"1234", studentCode:"20261002"},
      {id:"u_s3", role:"student", name:"Sofía López", email:"sofia.lopez@universidad.edu", password:"1234", studentCode:"20261003"},
      {id:"u_s4", role:"student", name:"Juan Pérez", email:"juan.perez@universidad.edu", password:"1234", studentCode:"20261004"}
    ],
    rooms:[
      {id:"r1", code:"B-201", name:"Bloque B · Aula 201", nfcToken:"NFC-B201"},
      {id:"r2", code:"A-105", name:"Bloque A · Aula 105", nfcToken:"NFC-A105"}
    ],
    courses:[
      {id:"c1", code:"MAT101", name:"Cálculo I"},
      {id:"c2", code:"ADM210", name:"Fundamentos de Administración"},
      {id:"c3", code:"ING150", name:"Programación I"}
    ],
    groups:[
      {id:"g1", courseId:"c1", name:"Grupo 01", professorId:"u_p1", roomId:"r1", schedule:"Lun/Mié 08:00-10:00"},
      {id:"g2", courseId:"c2", name:"Grupo 02", professorId:"u_p1", roomId:"r2", schedule:"Mar/Jue 14:00-16:00"},
      {id:"g3", courseId:"c3", name:"Grupo 01", professorId:"u_p2", roomId:"r1", schedule:"Vie 10:00-12:00"}
    ],
    enrollments:[
      {groupId:"g1", studentId:"u_s1"},{groupId:"g1", studentId:"u_s2"},{groupId:"g1", studentId:"u_s3"},
      {groupId:"g2", studentId:"u_s1"},{groupId:"g2", studentId:"u_s4"},
      {groupId:"g3", studentId:"u_s2"},{groupId:"g3", studentId:"u_s3"},{groupId:"g3", studentId:"u_s4"}
    ],
    attendanceSessions:[],
    attendanceRecords:[],
    audit:[]
  ...
  let db = load();
  let current = JSON.parse(sessionStorage.getItem(SESSION_KEY) || "null");
  ...
  function save(){
    ...
    if(channel) channel.postMessage("sync");
  }
  function audit(action, meta={}){
    db.audit.unshift({id:uid("a"), at:new Date().toISOString(), userId:current?.id||"system", action, meta});
  ...
  if(channel) channel.onmessage=()=>{db=load(); render();};

  function render(){
    db=load();
    if(!current) return renderLogin();
    const u=byId(db.users,current.id);
    if(!u){ logout(); return; }
    current=u;
  ...
  function renderLogin(){
      ...
      <div class="login-wrap">
        <div class="login-card">
          <section class="hero">
            ...
            <h1>FET Asistencias<br><span style="color:var(--accent)">Asistencias</span></h1>
    ...
    document.getElementById("loginBtn").onclick = ()=>{
      ...
      current={id:u.id}; sessionStorage.setItem(SESSION_KEY,JSON.stringify(current));
      const fromNFC = new URLSearchParams(location.search).has("nfc") || new URLSearchParams(location.search).has("room");
      currentView = fromNFC && u.role==="student" ? "checkin" : "dashboard";
      render();
    };
  }
  ...
  function logout(){ current=null; sessionStorage.removeItem(SESSION_KEY); currentView="dashboard"; render(); }
  ...
  function adminUsers(m){
    ...
    document.querySelectorAll("[data-del-user]").forEach(b=>b.onclick=()=>{
      ...
      db.groups.forEach(g=>{if(g.professorId===id) g.professorId="";});
      audit("Eliminar usuario",{id}); render();
    });
  }
  ...
  function showUserModal(){
    ...
    document.getElementById("muSave").onclick=()=>{
      ...
      db.users.push({id:uid("u"),name,email,password,role,studentCode});
      audit("Crear usuario",{email,role}); closeModal(); render();
    };
  }
  ...
  function adminCourses(m){
    ...
    document.addEventListener("click",function once(e){
      if(e.target?.id==="saveCourse"){
        ...
        db.courses.push({id:uid("c"),code,name}); audit("Crear curso",{code}); closeModal(); render(); document.removeEventListener("click",once);
      }
    });
  ...
  function showGroupModal(){
    ...
    document.getElementById("saveGroup").onclick=()=>{
      db.groups.push({id:uid("g"),courseId:val("gCourse"),name:val("gName"),professorId:val("gProf"),roomId:val("gRoom"),schedule:val("gSchedule")});
      audit("Crear grupo",{}); closeModal(); render();
    };
  }
  ...
  function showEnrollModal(groupId){
    ...
    document.getElementById("saveEnroll").onclick=()=>{
      db.enrollments=db.enrollments.filter(e=>e.groupId!==groupId);
      document.querySelectorAll("[data-student]:checked").forEach(ch=>db.enrollments.push({groupId,studentId:ch.dataset.student}));
      audit("Actualizar matrícula",{groupId}); closeModal(); render();
    };
  }
  ...
  function startSession(groupId){
    db=load();
    const g=byId(db.groups,groupId); if(!g) return;

    // Sanea cualquier sesión vieja o duplicada de este grupo.
    const nowMs=Date.now();
    db.attendanceSessions.forEach(s=>{
      if(s.groupId===groupId && s.status==="open"){
    ...
    const existing=db.attendanceSessions
      .filter(s=>s.groupId===groupId && s.status==="open" && new Date(s.endsAt).getTime()>=nowMs)
      .sort((a,b)=>new Date(b.startedAt)-new Date(a.startedAt))[0];

    if(existing){
      return toast("Ya existe una asistencia activa para este grupo. PIN: "+existing.pin);
    }

    const pin=String(Math.floor(1000+Math.random()*9000));
    const now=new Date();
    const end=new Date(now.getTime()+db.university.sessionMinutes*60000);
    const s={
      id:uid("s"),
      groupId,
      roomId:g.roomId,
    ...
    db.attendanceSessions.push(s);
    audit("Iniciar asistencia",{sessionId:s.id,groupId,pin});
    render();
  }
  function closeSession(id){
    const s=byId(db.attendanceSessions,id); if(!s)return;
    s.status="closed";s.closedAt=new Date().toISOString(); audit("Cerrar asistencia",{sessionId:id}); render();
  }
  ...
  function showSessionDetail(sessionId){
    ...
    document.getElementById("saveStatuses").onclick=()=>{
      document.querySelectorAll("[data-change-status]").forEach(sel=>{
        const studentId=sel.dataset.changeStatus, status=sel.value;
        let r=db.attendanceRecords.find(x=>x.sessionId===s.id&&x.studentId===studentId);
        if(status==="absent"){
          if(r) db.attendanceRecords=db.attendanceRecords.filter(x=>x.id!==r.id);
        } else {
          if(!r){r={id:uid("ar"),sessionId:s.id,studentId,status,checkedAt:new Date().toISOString(),method:"manual"};db.attendanceRecords.push(r);}
          else r.status=status;
        }
      });
      audit("Editar asistencia manual",{sessionId:s.id}); closeModal(); render();
    };
  }
...
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
  ...
  function checkin(sessionId, rawPin){
    // Capturamos el PIN ANTES de volver a leer la base.
    const entered=String(rawPin ?? "").normalize("NFKC").replace(/[^0-9]/g,"");

    if(entered.length!==4){
      return toast("Ingresa exactamente los 4 dígitos del PIN");
    }

    db=load();

    const candidates=db.attendanceSessions
    ...
    if(!s){
      return toast("No se encontró esta asistencia. Actualiza la página.");
    }

    if(s.status!=="open" || !Number.isFinite(new Date(s.endsAt).getTime()) || new Date(s.endsAt).getTime()<Date.now()){
      return toast("La asistencia ya no está activa");
    }

    if(!db.enrollments.some(e=>e.groupId===s.groupId&&e.studentId===current.id)){
      return toast("No perteneces a este curso");
    }

    if(db.attendanceRecords.some(r=>r.sessionId===sessionId&&r.studentId===current.id)){
      return toast("Ya registraste asistencia en este curso");
    }

    const expected=String(s.pin ?? "").normalize("NFKC").replace(/[^0-9]/g,"");

    // Comparación estricta únicamente de los 4 dígitos.
    if(expected.length!==4){
      return toast("El PIN de esta sesión está dañado. El profesor debe cerrar e iniciar una nueva asistencia.");
    }

    if(entered!==expected){
      return toast("PIN incorrecto. Verifica que sea el PIN del curso que estás marcando.");
    }

    const elapsed=(Date.now()-new Date(s.startedAt).getTime())/60000;
    const status=elapsed<=db.university.graceMinutes?"present":"late";
    ...
    db.attendanceRecords.push({
      ...
      method:"plataforma+pin"
    });

    audit("Registrar asistencia",{sessionId:s.id,status});
  ...
  // Mantiene las sesiones expiradas coherentes
  function expire(){
    let changed=false; const now=Date.now();
    db.attendanceSessions.forEach(s=>{if(s.status==="open"&&new Date(s.endsAt).getTime()<now){s.status="closed";s.closedAt=s.endsAt;changed=true;}});
    if(changed) save();
  }
  setInterval(()=>{
    expire();
    const typing=document.activeElement && ["INPUT","SELECT","TEXTAREA"].includes(document.activeElement.tagName);
    if(current && !typing) render();
  },5000);

  window.FET Asistencias = {
    reset(){localStorage.removeItem(KEY);sessionStorage.removeItem(SESSION_KEY);location.href=location.href.split("?")[0];},
    exportBackup(){
      const blob=new Blob([JSON.stringify(db,null,2)],{type:"application/json"}),a=document.createElement("a");
      a.href=URL.createObjectURL(blob);a.download="fet_asistencia_backup.json";a.click();URL.revokeObjectURL(a.href);
    }
  };
